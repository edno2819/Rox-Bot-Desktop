...
def set_variables_configs2(entrada, delay, stop_win, stop_loss, type_operation, type_stop):
    ...
    MAIN_RUN.set_configs({'ENTRADA':abs(float(entrada)), 'DELAY':int(delay), 'stop_win':float(stop_win), 'stop_loss':float(stop_loss), 'type_operation':type_operation, 'type_stop':type_stop})
    MAIN_RUN.set_init()